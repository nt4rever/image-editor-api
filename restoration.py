...
def matrix(igray):
    ...
    for i in range(igray.shape[0]):
        for j in range(igray.shape[1]):
            img_1[i + int((filter.shape[0] - 1) // 2), j +
                  int((filter.shape[1] - 1) // 2)] = igray[i, j]
    return img_1
...
def tb_harmonic(path):
    ...
    for i in range(img.shape[0]):
        for j in range(img.shape[1]):
            temp = img_1[i:i+filter.shape[0], j:j+filter.shape[1]]
            res = (filter.shape[0] * filter.shape[1]
                   ) // np.sum(np.reciprocal(temp))
            img_1_res[i, j] = res
    return img_1_res
...
def loc_alpha(path):
    ...
    for i in range(img.shape[0]):
        for j in range(img.shape[1]):
            ...
            # max
            indices = temp.argpartition(
                temp.size - (d//2), axis=None)[-(d//2):]
